...
#define SSID_NAME  "400MEGANBYTES"			//<ENTER SSID HERE>
#define PASSKEY    "gentlemango275" 	//<ENTER PASSWORD HERE>
...
void ESP8266SendBuffer(const char* inputBuf, uint32_t pack_size) {
	int index = 0;
	while(index < pack_size) {
		ESP8266_PrintChar(inputBuf[index++]);
	}
}
...
//  SearchStart("no change");//AT+GMR version 0018000902
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWMODE_DEF=%d\r\n", mode); //Sets the default Wi-Fi mode in the Flash
...
int ESP8266_JoinAccessPoint(const char* ssid, const char* password){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWJAP_DEF=\"%s\",\"%s\"\r\n", ssid, password); //Connects to an AP, save config in the Flash
    ESP8266SendCommand((const char*)TXBuffer);
...
// output: 1 if success, 0 if fail
int ESP8266_MakeTCPConnection(char *IPaddress, int port){
  int try=MAXTRY;
  SearchStart("ok");
  while(try){
...
// output: 1 if success, 0 if fail
int ESP8266_SendTCP(char* fetch, uint32_t pack_size){
  volatile uint32_t time,n;
  sprintf((char*)TXBuffer, "AT+CIPSEND=%d\r\n", pack_size); //Sends Data
  ESP8266SendCommand(TXBuffer);
  DelayMs(50);
  ESP8266SendBuffer(fetch, pack_size);
  ServerResponseSearchStart();
  n = 8000;
